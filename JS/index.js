document.addEventListener('DOMContentLoaded', () => {
    ...
    const initScrollAnimations = () => {
      ...
      document.querySelectorAll('section:not(.hero-section)').forEach((section, index) => {
        ...
        const cta = section.querySelector('.connect-cta');
      ...
      // Анимации карточек преимуществ с волновым эффектом
      ...
      directionCards.forEach((card, index) => {
        ...
        // Parallax для изображений направлений
        const directionImage = card.querySelector('.direction-image');
        if (directionImage) {
          gsap.to(directionImage, {
            y: -30,
            ease: 'none',
            scrollTrigger: {
              trigger: card,
    ...
    UI.initHoverEffects = () => {
      ...
      const cards = document.querySelectorAll('.advantage-card, .direction-card');
      cards.forEach(card => {
